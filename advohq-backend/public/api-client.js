...
(function (global) {
      ...
      if (!refreshPromise) {
        refreshPromise = fetch(`${API_URL}/auth/refresh`, {
          ...
          .catch(() => {
            ...
            global.location.href = '/login2.html';
          })
  ...
  const files = {
    ...
    async upload(caseId, file, onProgress) {
      ...
      // 3. Update case with s3Key and fileSize
  ...
  global.AdvoAPI = { auth, cases, events, files, _fetch: apiFetch };

  // ── Auth guard (call on protected pages) ──────────────────────────────────

  global.requireAuth = function () {
    if (!auth.isLoggedIn()) {
      global.location.href = '/login2.html?next=' + encodeURIComponent(global.location.pathname);
    }
  };
